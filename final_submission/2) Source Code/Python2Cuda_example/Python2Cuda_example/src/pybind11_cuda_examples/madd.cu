...
// Perform matrix multiplication on GPU using global memory
...
// Perform matrix multiplication on GPU using shared memory
__global__ void matrix_multiplication_shared_memory(int* input1, int* input2, int* output)
{
    ...
    for (int s = 0; s < tile_size; s++) {
        ...
        // load tiles A, B into shared memory
    ...
    // transfer result from shared to global memory
    if (row < N && col < N) {
        output[row * N + col] = sum;
    }
}
